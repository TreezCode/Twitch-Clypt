...
const getClips = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!gameResponse) {
      ...
      if (clipData.length > 0) {
        try {
          ...
          console.log(
            `Successfully added ${numAdded} of ${twitchName}'s clips to the database`
              .yellow
          )
        } catch (error) {
          ...
          console.log(
            `Successfully updated ${numUpdated} of ${twitchName}'s clips in the database`
              .yellow
          )
        } finally {
    ...
    if (!twitchResponse) {
      ...
      if (clipData.length > 0) {
        try {
          ...
          let numAdded = clipData.length
          console.log(
            `Successfully added ${numAdded} ${gameName} Twitch clips to the database`
              .yellow
          )
        } catch (error) {
          if (!error.message.includes('E11000')) {
          ...
          let numUpdated = error.result.result.nInserted
          console.log(
            `Successfully updated ${numUpdated} ${gameName} Twitch clips in the database`
              .yellow
          )
        } finally {
    ...
    if (twitchResponse && gameResponse) {
      ...
      const twitchClipResponse = await axios.get(
        process.env.GET_CLIPS,
        twitchOptions
      )
      const gameClipResponse = await axios.get(
        process.env.GET_CLIPS,
        gameOptions
      )
      await Promise.all([twitchClipResponse, gameClipResponse]).then(
        (result) => result
      )
      ...
      if (twitchClipData.length > 0 && gameClipData.length > 0) {
        try {
          // update database with new set 'ordered:false'
          const updateTwitchClips = await Clip.insertMany(twitchClipData, {
            ordered: false,
          })
          const updateGameClips = await Clip.insertMany(gameClipData, {
            ordered: false,
          })
          await Promise.all([updateTwitchClips, updateGameClips]).then(
            (result) => result
          )
          let numTwitchAdded = twitchClipData.length
          let numGameAdded = gameClipData.length
          console.log(
            `Successfully added ${numTwitchAdded} of ${twitchName}'s Twitch clips to the database`
              .yellow
          )
          console.log(
            `Successfully added ${numGameAdded} ${gameName} Twitch clips to the database`
              .yellow
          )
        } catch (error) {
          if (!error.message.includes('E11000')) {
      ...
      if (twitchClipData.length > 0) {
        try {
          ...
          console.log(
            `Successfully added ${numAdded} of ${twitchName}'s Twitch clips to the database`
              .yellow
          )
        } catch (error) {
          if (!error.message.includes('E11000')) {
          ...
          let numUpdated = error.result.result.nInserted
          console.log(
            `Successfully updated ${numUpdated} of ${twitchName}'s Twitch clips in the database`
              .yellow
          )
        } finally {
...
const saveClip = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const clipExists = loggedIn.clips.find((clip) => clip.slug === slug)
    if (clipExists) {
      res.status(400)
      throw Error(`Clip has already been saved`)
    }
    const clip = await Clip.findOne({ id: slug })
    ...
    console.log(
      `${loggedIn.name} saved one of ${clip.broadcaster_name}'s clips`.yellow
    )
...
const unsaveClip = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const updatedUser = await User.findByIdAndUpdate(
      loggedIn._id,
      { $pull: { clips: { slug: slug } } },
      { new: true }
    )
    console.log(
      `${loggedIn.name} deleted one of ${clipExists.name}'s clips`.yellow
    )
